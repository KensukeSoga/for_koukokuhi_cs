 ...
 * @author S.Fujimoto
 */
public class FindHMBillCreationCrDAO extends AbstractRdbDao {

    /** 検索条件 */
    private FindHMBillCreationCrCondition _condition;

    /**
     * デフォルトコンストラクタ
     */
    public FindHMBillCreationCrDAO() {
    ...
    public List<FindHMBillCreationCrVO> selectVO(FindHMBillCreationCrCondition condition) throws HAMException {

        //パラメータチェック
        if (condition == null) {
            throw new HAMException("検索エラー", "BJ-E0001");
        }
        super.setModel(new FindHMBillCreationCrVO());

        try {
            _condition = condition;
            return (List<FindHMBillCreationCrVO>) super.find();
