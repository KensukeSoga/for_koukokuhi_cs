 ...
 * @author S.Fujimoto
 */
public class FindHMBillCreationCaptureDAO extends AbstractRdbDao {

    /** 検索条件 */
    private FindHMBillCreationCaptureCondition _condition = null;

    /**
     * デフォルトコンストラクタ
     */
    public FindHMBillCreationCaptureDAO() {
    ...
    public List<FindHMBillCreationCaptureVO> selectVO(FindHMBillCreationCaptureCondition condition) throws HAMException {

        //パラメータチェック
        if (condition == null) {
            throw new HAMException("検索エラー", "BJ-E0001");
        }
        super.setModel(new FindHMBillCreationCaptureVO());

        try {
            _condition = condition;
            return (List<FindHMBillCreationCaptureVO>) super.find();
