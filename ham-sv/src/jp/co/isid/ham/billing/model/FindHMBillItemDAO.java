 ...
 * @author S.Fujimoto
 */
public class FindHMBillItemDAO extends AbstractRdbDao {

    /** 検索条件 */
    private FindHMBillItemCondition _condition = null;

    /**
     * デフォルトコンストラクタ
     */
    public FindHMBillItemDAO() {
    ...
    public String getSelectSQL() {
        ...
        sql.append(" NVL(SUM(" + Tbj06EstimateDetail.MITUMORI + "), 0) " + Tbj06EstimateDetail.MITUMORI + ",");
        sql.append(" TO_CHAR(" + Tbj05EstimateItem.CRDATE + ", 'MM') || '月-' || TRIM(TO_CHAR(" + Tbj05EstimateItem.ESTSEQNO + ", '0000')) || '-' || TO_CHAR(" + Tbj05EstimateItem.COOPKBN + ") ESTIMATENO,");
    ...
    public List<FindHMBillItemVO> selectVO(FindHMBillItemCondition condition) throws HAMException {

        //パラメータチェック
        if (condition == null) {
            throw new HAMException("検索エラー", "BJ-E0001");
        }
        super.setModel(new FindHMBillItemVO());

        try {
            _condition = condition;
            return (List<FindHMBillItemVO>) super.find();
